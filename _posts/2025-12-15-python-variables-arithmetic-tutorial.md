...
J = input("請輸入第二個數字: ")
result = I + J
print(result)  # 輸出: 如果輸入 100 和 200，結果為 100200
```
...
result = I + J
print(result)  # 輸出: 300  (假設輸入 100 和 200)
```
...
H = int(input("請輸入身高(公分): ")) / 100  # 將公分轉換為公尺
BMI = W / (H ** 2)
print(f"BMI: {int(BMI)}")  # 將小數取整
```
